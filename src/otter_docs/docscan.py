...
# Docs where "staleness" is a category error — never judged. Two classes:
#   generated:  a machine writes them from the code (docs/generated/, or a
#               generated-marker in the head) — the fix is the generator, not
#               the doc, so a judge call is pure waste.
#   append-only history: chronological logs + dated audits — a 2026-05 entry is
#               a record of what was true THEN, not a current claim about code.
# Matched (glob) against the repo-relative doc path; case-insensitive on name.
DEFAULT_DOC_EXCLUDES: tuple[str, ...] = (
    "docs/generated/**",
    "**/BUILD_LOG.md",
    "**/WORKLOG.md",
    "**/CHANGELOG*.md",
...
# A doc whose first _GENERATED_SCAN_BYTES contains one of these (case-folded)
# is treated as generated and skipped.
_GENERATED_MARKERS = (
    "auto-generated",
    "autogenerated",
    "do not edit",
...
# A doc filename carrying an ISO date (YYYY-MM-DD) is a dated record/audit —
# append-only history, not a living claim.
_DATED_NAME_RE = re.compile(r"\d{4}-\d{2}-\d{2}")
...
def _extract_tokens(section_text: str) -> list[str]:
    """All mention tokens from a section, in order (fences removed).

    NOT de-duplicated: repeated mentions are what the prominence gate counts
    (a symbol named ≥2× is a claim, not a reference in passing), so the caller
    needs every occurrence. Symbol-level dedup happens downstream."""
...
def _should_skip_doc(rel: str, text: str, excludes: tuple[str, ...]) -> str | None:
    """Return a reason string if this doc must not be judged, else None.

    Skips generated docs (path glob, or a generated-marker in the head) and
    dated/append-only history — see DEFAULT_DOC_EXCLUDES. Judging those is a
    category error: the fix is the generator, or the record is history."""
    from otter_docs.discovery import is_test_path

    name = rel.rsplit("/", 1)[-1]
    if is_test_path(rel):
        return "test-path"
    if _DATED_NAME_RE.search(name):
        return "dated-record"
    for pat in excludes:
        if PurePosixPath(rel).match(pat) or PurePosixPath(name).match(pat):
            return f"excluded:{pat}"
    head = text[:_GENERATED_SCAN_BYTES].lower()
...
def build_doc_findings(
    ...
    *,
    ...
    doc_excludes: tuple[str, ...] = DEFAULT_DOC_EXCLUDES,
    ...
    Generated + dated/history docs are skipped (`_should_skip_doc`).
    """
    ...
    for pattern in doc_globs:
        for path in sorted(repo_root.glob(pattern)):
            ...
            try:
                ...
                continue
            skip = _should_skip_doc(rel, doc_text, doc_excludes)
            if skip is not None:
                n_skipped += 1
                logger.debug("doc.divergence: skipping %s (%s)", rel, skip)
                continue
            findings.extend(
    ...
    if n_skipped:
        logger.info(
            "doc.divergence: skipped %d generated/dated/test docs", n_skipped
        )
    return findings
...
def _findings_for_doc(
    ...
    for section in split_sections(doc_text):
        ...
        # Tally mentions per resolved symbol (before dedup) — a symbol named
        # once among twenty in a list is a reference in passing, not a claim
        # about it. Keep only *prominent* symbols: named in the heading, or
        # mentioned ≥2×. This is the cheap precision gate before the LLM.
        counts: dict[str, int] = {}
        rec: dict[str, tuple[str, str, str, str]] = {}
        for tok in _extract_tokens(section.text):
            ...
            rec.setdefault(key, hit)
        resolved: list[tuple[str, str, str, str]] = []
        for key, hit in rec.items():
            name, sym_path = hit[2], hit[3]
            # A doc claim about test code isn't the doc-audit signal (OD-2
            # test/prod separation) — the target is prose about the system.
            if is_test_path(sym_path):
                continue
            in_heading = (
                name.lower() in heading_low
                or PurePosixPath(sym_path).name.lower() in heading_low
            )
            if counts[key] >= 2 or in_heading:
                resolved.append(hit)
        if not resolved:
            continue
