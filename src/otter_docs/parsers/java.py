"""Java source parser via tree-sitter.

Extracts:
  - ModuleRecord (one per .java file)
  - FunctionRecord for each `method_declaration` and `constructor_declaration`,
    namespaced as `ClassName.methodName` when nested under a type.
  - ClassRecord for each `class_declaration`, `interface_declaration`,
    `enum_declaration`, `record_declaration`.
  - IMPORTS edges from `import a.b.c;` statements.
  - Intra-file CALLS edges by callee identifier match.

Constructors keep their original name (the class name itself). Methods
inside nested types use the immediate enclosing type — we don't flatten
deep nesting into the qualified name to keep edges legible.
"""

from __future__ import annotations

import hashlib

import tree_sitter_java
from tree_sitter import Language, Node, Parser

from otter_docs.guids import resolve_guid
from otter_docs.models import (
    ClassRecord,
    Edge,
    FunctionRecord,
    ModuleRecord,
)
from otter_docs.models import (
    Language as Lang,
)
from otter_docs.parsers.base import ParseResult, register

_LANGUAGE = Language(tree_sitter_java.language())
_PARSER = Parser(_LANGUAGE)


def _guid(repo: str, path: str, name: str, line: int) -> str:
    h = hashlib.blake2b(digest_size=12)
    h.update(f"{repo}|{path}|{name}|{line}".encode())
    return h.hexdigest()


def _name(node: Node, field: str = "name") -> str:
    n = node.child_by_field_name(field)
    return n.text.decode("utf-8", errors="replace") if n else ""


def _imports(root: Node) -> list[str]:
    out: list[str] = []
    for child in root.named_children:
        if child.type != "import_declaration":
            continue
        # The import path is the only non-modifier named child.
        for c in child.named_children:
            if c.type in ("scoped_identifier", "identifier"):
                out.append(c.text.decode("utf-8", errors="replace"))
                break
            if c.type == "asterisk":
                continue
    return out


def _args(node: Node) -> list[str]:
    """Extract parameter names from a method/constructor's parameters field."""
    params = node.child_by_field_name("parameters")
    if params is None:
        return []
    out: list[str] = []
    for p in params.named_children:
        if p.type not in ("formal_parameter", "spread_parameter"):
            continue
        ident = p.child_by_field_name("name")
        if ident is not None:
            out.append(ident.text.decode("utf-8", errors="replace"))
    return out


def _collect_calls(body: Node | None, sink: list[str]) -> None:
    if body is None:
        return
    for c in body.named_children:
        if c.type == "method_invocation":
            name = c.child_by_field_name("name")
            if name is not None:
                sink.append(name.text.decode("utf-8", errors="replace"))
        elif c.type == "object_creation_expression":
            type_node = c.child_by_field_name("type")
            if type_node is not None:
                sink.append(type_node.text.decode("utf-8", errors="replace"))
        _collect_calls(c, sink)


_TYPE_NODES = (
    "class_declaration",
    "interface_declaration",
    "enum_declaration",
    "record_declaration",
)


class JavaParser:
    language = Lang.JAVA

    def parse(self, *, repo: str, path: str, source: bytes) -> ParseResult:
        tree = _PARSER.parse(source)
        root = tree.root_node
        module = ModuleRecord(
            repo=repo, path=path, language=Lang.JAVA, imports=_imports(root)
        )

        functions: list[FunctionRecord] = []
        classes: list[ClassRecord] = []
        edges: list[Edge] = []
        pending_calls: list[tuple[str, str]] = []

        def emit_fn(node: Node, qualified_name: str) -> None:
            line = node.start_point.row + 1
            end = node.end_point.row + 1
            guid = resolve_guid(
                source, node.start_point.row,
                _guid(repo, path, qualified_name, line),
            )
            functions.append(FunctionRecord(
                repo=repo, guid=guid, name=qualified_name, module_path=path,
                line=line, end_line=end, args=_args(node),
            ))
            body = node.child_by_field_name("body")
            if body is not None:
                call_names: list[str] = []
                _collect_calls(body, call_names)
                for cn in call_names:
                    pending_calls.append((guid, cn))

        def emit_class(node: Node) -> str:
            name = _name(node)
            if not name:
                return ""
            line = node.start_point.row + 1
            end = node.end_point.row + 1
            guid = resolve_guid(
                source, node.start_point.row,
                _guid(repo, path, f"class:{name}", line),
            )
            classes.append(ClassRecord(
                repo=repo, guid=guid, name=name, module_path=path,
                line=line, end_line=end,
            ))
            return name

        def walk_type(node: Node, enclosing: str) -> None:
            type_name = emit_class(node)
            body = node.child_by_field_name("body")
            if body is None:
                return
            for member in body.named_children:
                if member.type == "method_declaration":
                    method_name = _name(member)
                    qualified = (
                        f"{type_name}.{method_name}" if type_name else method_name
                    )
                    emit_fn(member, qualified)
                elif member.type == "constructor_declaration":
                    method_name = _name(member)
                    qualified = (
                        f"{type_name}.{method_name}" if type_name else method_name
                    )
                    emit_fn(member, qualified)
                elif member.type in _TYPE_NODES:
                    walk_type(member, type_name)

        for child in root.named_children:
            if child.type in _TYPE_NODES:
                walk_type(child, "")

        by_name: dict[str, str] = {}
        for fn in functions:
            by_name.setdefault(fn.name, fn.guid)
            if "." in fn.name:
                bare = fn.name.split(".", 1)[1]
                by_name.setdefault(bare, fn.guid)
        for caller, callee_name in pending_calls:
            target = by_name.get(callee_name)
            if target is not None and target != caller:
                edges.append(Edge(kind="CALLS", src_id=caller, dst_id=target))

        for imp in module.imports:
            edges.append(Edge(kind="IMPORTS", src_id=path, dst_id=imp))

        return ParseResult(
            module=module, functions=functions, classes=classes, edges=edges,
        )


register(JavaParser())
